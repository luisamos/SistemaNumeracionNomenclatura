...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Condicion_Limite : SubFicha
    {
        /// <summary>
        /// Condición de Limite
        /// </summary>
        internal Condicion_Limite()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cCondicion_Limite()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_CONDICION_LIMITE";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                condicion_limite.Codigo = codigo_txt.Text;
                condicion_limite.Descripcion = descripcion_txt.Text;
                condicion_limite.Usuario = Usuario;
                i = condicion_limite.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, el código de condición de limite: '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                condicion_limite.Descripcion = descripcion_txt.Text;
                condicion_limite.Usuario = Usuario;
                i = condicion_limite.modificar();
                if (i > 0) MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente, el código condición de limite esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void eliminar()
        {
            ...
            condicion_limite.Codigo = codigo_txt.Text;
            condicion_limite.Usuario = Usuario;
            i = condicion_limite.eliminar();
            if (i >= 2) MessageBox.Show("Se a eliminado el Registro.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else MessageBox.Show("Error, No se puede eliminar, el código condición de limite esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_CONDICION_LIMITE.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new condicion_limite(), tabla);
                else reporte = new Reporte(new condicion_limite(), FICHA_ds);
