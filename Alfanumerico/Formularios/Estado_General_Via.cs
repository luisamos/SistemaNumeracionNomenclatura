...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Estado_General_Via : SubFicha
    {
        internal Estado_General_Via()
        {
            InitializeComponent();
        ...
        void listar()
        {
            ...
            FICHA_ds = (new cEstado_General_Via()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_ESTADO_GENERAL_VIA";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                estado_general_via.Codigo = codigo_txt.Text;
                estado_general_via.Descripcion = descripcion_txt.Text;
                estado_general_via.Usuario = Usuario;
                i = estado_general_via.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);                   
                else MessageBox.Show("Error, el código de estado general de Vía : '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                estado_general_via.Descripcion = descripcion_txt.Text;
                estado_general_via.Usuario = Usuario;
                i = estado_general_via.modificar();
                if (i > 0) MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente, el estado general de la Vía esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void eliminar()
        {
            ...
            estado_general_via.Codigo = codigo_txt.Text;
            estado_general_via.Usuario = Usuario;
            i = estado_general_via.eliminar();
            if (i >= 2)MessageBox.Show("Se a eliminado el Registro", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else MessageBox.Show("Error, No se puede eliminar, el estado general de la Vía esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_ESTADO_GENERAL_VIA.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new estado_general_via(), tabla);
                else reporte = new Reporte(new estado_general_via(), FICHA_ds);
