...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Estado_Llenado_Ficha : SubFicha
    {
        internal Estado_Llenado_Ficha()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cEstado_Llenado_Ficha()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_ESTADO_LLENADO_FICHA";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                estado_llenado_ficha.Codigo = codigo_txt.Text;
                estado_llenado_ficha.Descripcion = descripcion_txt.Text;
                estado_llenado_ficha.Usuario = Usuario;
                i = estado_llenado_ficha.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, el Código de Estado general de ficha : '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                estado_llenado_ficha.Descripcion = descripcion_txt.Text;
                estado_llenado_ficha.Usuario = Usuario;
                i = estado_llenado_ficha.modificar();
                if (i > 0) MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente, el Estado general de la ficha esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void eliminar()
        {
            ...
            estado_llenado_ficha.Codigo = codigo_txt.Text;
            estado_llenado_ficha.Usuario = Usuario;
            i = estado_llenado_ficha.eliminar();
            if (i >= 2) MessageBox.Show("Se a eliminado el Registro.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else MessageBox.Show("Error, No se puede eliminar, el Estado general de la ficha esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_ESTADO_LLENADO_FICHA.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new estado_llenado_ficha(), tabla);
                else reporte = new Reporte(new estado_llenado_ficha(), FICHA_ds);
