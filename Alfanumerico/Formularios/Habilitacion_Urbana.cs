...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Habilitacion_Urbana : SubFicha
    {
        internal Habilitacion_Urbana()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cHabilitacion_Urbana()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_HABILITACION_URBANA";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                habilitacion_urbana.Codigo = codigo_txt.Text;
                habilitacion_urbana.Descripcion = descripcion_txt.Text;
                habilitacion_urbana.Usuario = Usuario;
                i = habilitacion_urbana.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, el habilitacion_urbana : '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                habilitacion_urbana.Descripcion = descripcion_txt.Text;
                habilitacion_urbana.Usuario = Usuario;
                i = habilitacion_urbana.modificar();
                if (i > 0) MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente, habilitación urbana esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void eliminar()
        {
            ...
            habilitacion_urbana.Codigo = codigo_txt.Text;
            habilitacion_urbana.Usuario = Usuario;
            i = habilitacion_urbana.eliminar();
            if (i >= 2) MessageBox.Show("Se a eliminado el Registro.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else MessageBox.Show("Error, No se puede eliminar, habilitación urbana esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_HABILITACION_URBANA.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new habilitacion_urbana(), tabla);
                else reporte = new Reporte(new habilitacion_urbana(), FICHA_ds);
