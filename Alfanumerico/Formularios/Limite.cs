...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Limite : SubFicha
    {
        internal Limite()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cLimite()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_LIMITE";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                limite.Codigo = codigo_txt.Text;
                limite.Descripcion = descripcion_txt.Text;
                ...
                else MessageBox.Show("Error, el código de limite : '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                else MessageBox.Show("Error, no se a Modificado Correctamente, el código de limite esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void eliminar()
        {
            ...
            else MessageBox.Show("Error, No se puede eliminar, el código de limite esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();       
        }
        #endregion
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_LIMITE.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new limite(), tabla);
                else reporte = new Reporte(new limite(), FICHA_ds);
