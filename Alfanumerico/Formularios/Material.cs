...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Material : SubFicha
    {
        internal Material()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cMaterial_Componente_Urbano_Mobiliario_Urbano()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_MATERIAL";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                material_componente_urbano_mobiliario_urbano.Codigo = codigo_txt.Text;
                material_componente_urbano_mobiliario_urbano.Descripcion = descripcion_txt.Text;
                material_componente_urbano_mobiliario_urbano.Usuario = Usuario;
                i = material_componente_urbano_mobiliario_urbano.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, el Código de material : '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                material_componente_urbano_mobiliario_urbano.Descripcion = descripcion_txt.Text;
                material_componente_urbano_mobiliario_urbano.Usuario = Usuario;
                i = material_componente_urbano_mobiliario_urbano.modificar();

                if (i > 0) MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente, material esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();       
            }
        }
        void eliminar()
        {
            ...
            material_componente_urbano_mobiliario_urbano.Codigo = codigo_txt.Text;
            material_componente_urbano_mobiliario_urbano.Usuario = Usuario;
            i = material_componente_urbano_mobiliario_urbano.eliminar();
            if (i >= 2) MessageBox.Show("Se a eliminado el Registro", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);            
            else MessageBox.Show("Error, No se puede eliminar, material esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_MATERIAL.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new material(), tabla);
                else reporte = new Reporte(new material(), FICHA_ds);
