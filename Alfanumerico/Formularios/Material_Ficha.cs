...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Material_Ficha : SubFicha
    {
        internal Material_Ficha()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cMaterial()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_MATERIAL_FICHA";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                material.Codigo = codigo_txt.Text;
                material.Descripcion = descripcion_txt.Text;
                ...
                else MessageBox.Show("Error, el código de material de ficha: '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_MATERIAL_FICHA.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new material_ficha(), tabla);
                else reporte = new Reporte(new material_ficha(), FICHA_ds);
