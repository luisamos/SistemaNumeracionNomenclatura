...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Tipo_Equipamiento_Urbano : SubFicha
    {
        internal Tipo_Equipamiento_Urbano()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cTipo_Equipamiento_Urbano()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_TIPO_EQUIPAMIENTO_URBANO";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                tipo_equipamiento_urbano.Codigo = codigo_txt.Text;
                tipo_equipamiento_urbano.Descripcion = descripcion_txt.Text;
                tipo_equipamiento_urbano.Usuario = Usuario;
                i = tipo_equipamiento_urbano.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, el tipo de equipamiento urbano : '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error); 
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                tipo_equipamiento_urbano.Descripcion = descripcion_txt.Text;
                tipo_equipamiento_urbano.Usuario = Usuario;
                i = tipo_equipamiento_urbano.modificar();
                if (i > 0) MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente, el tipo de equipamiento urbano esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();       
            }
        }
        void eliminar()
        {
            ...
            tipo_equipamiento_urbano.Codigo = codigo_txt.Text;
            tipo_equipamiento_urbano.Usuario = Usuario;
            i = tipo_equipamiento_urbano.eliminar();
            if (i >= 2) MessageBox.Show("Se a eliminado el Registro", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else MessageBox.Show("Error, No se puede eliminar, el tipo de equipamiento urbano esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_TIPO_EQUIPAMIENTO_URBANO.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new tipo_equipamiento_urbano(), tabla);
                else reporte = new Reporte(new tipo_equipamiento_urbano(), FICHA_ds);
