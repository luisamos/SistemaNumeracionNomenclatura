...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Tipo_Espacio_Recreacion : SubFicha
    {
        internal Tipo_Espacio_Recreacion()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cTipo_Espacio_Recreacion()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_TIPO_ESPACIO_RECREACION";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                tipo_espacio_recreacion.Codigo = codigo_txt.Text;
                tipo_espacio_recreacion.Descripcion = descripcion_txt.Text;
                tipo_espacio_recreacion.Usuario = Usuario;
                i = tipo_espacio_recreacion.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);                    
                else MessageBox.Show("Error, el Código de tipo de recreación: '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                tipo_espacio_recreacion.Descripcion = descripcion_txt.Text;
                tipo_espacio_recreacion.Usuario = Usuario;
                i = tipo_espacio_recreacion.modificar();
                if (i > 0) MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente, el tipo de espacio de recreación esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void eliminar()
        {
            ...
            tipo_espacio_recreacion.Codigo = codigo_txt.Text;
            tipo_espacio_recreacion.Usuario = Usuario;
            i = tipo_espacio_recreacion.eliminar();
            if (i >= 2) MessageBox.Show("Se a eliminado el Registro", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else MessageBox.Show("Error, No se puede eliminar, el tipo de espacio de recreación esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_TIPO_ESPACIO_RECREACION.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new tipo_espacio_recreacion(), tabla);
                else reporte = new Reporte(new tipo_espacio_recreacion(), FICHA_ds);
