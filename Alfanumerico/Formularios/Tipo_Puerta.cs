...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Tipo_Puerta : SubFicha
    {
        internal Tipo_Puerta()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cTipo_Puerta()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_TIPO_PUERTA";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                tipo_puerta.Codigo = codigo_txt.Text;
                tipo_puerta.Descripcion = descripcion_txt.Text;
                tipo_puerta.Usuario = Usuario;
                i = tipo_puerta.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);         
                else MessageBox.Show("Error, el Código de  puerta: '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                tipo_puerta.Descripcion = descripcion_txt.Text;
                tipo_puerta.Usuario = Usuario;
                i = tipo_puerta.modificar();
                if (i > 0)MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente,el tipo de puerta esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void eliminar()
        {
            ...
            tipo_puerta.Codigo = codigo_txt.Text;
            tipo_puerta.Usuario = Usuario;
            i = tipo_puerta.eliminar();
            if (i >= 2) MessageBox.Show("Se a eliminado el Registro.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);           
            else  MessageBox.Show("Error, No se puede eliminar, el tipo de puerta esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void listar_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                codigo_txt.DataBindings.Add(new Binding("Text", SUBFICHA_bs, "TIPO DE PUERTA", true));
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_TIPO_PUERTA.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new tipo_puerta(), tabla);
                else reporte = new Reporte(new tipo_puerta(), FICHA_ds);
