...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Tipo_Uso_Resaltante : SubFicha
    {
        internal Tipo_Uso_Resaltante()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cTipo_Uso_Resaltante()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_TIPO_USO_RESALTANTE";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                tipo_uso_resaltante.Codigo = codigo_txt.Text;
                tipo_uso_resaltante.Descripcion = descripcion_txt.Text;
                tipo_uso_resaltante.Usuario = Usuario;
                i = tipo_uso_resaltante.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, el Tipo de uso resaltante : '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                tipo_uso_resaltante.Descripcion = descripcion_txt.Text;
                tipo_uso_resaltante.Usuario = Usuario;
                i = tipo_uso_resaltante.modificar();
                if (i > 0) MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente,  el tipo de uso resaltante esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void eliminar()
        {
            ...
            tipo_uso_resaltante.Codigo = codigo_txt.Text;
            tipo_uso_resaltante.Usuario = Usuario;
            i = tipo_uso_resaltante.eliminar();
            if (i >= 2) MessageBox.Show("Se a eliminado el Registro.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else MessageBox.Show("Error, No se puede eliminar, el tipo de uso resaltente esta en uso y/o esta Desabilitada", "Incorrecto.", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_TIPO_USO_RESALTANTE.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new tipo_uso_resaltante(), tabla);
                else reporte = new Reporte(new tipo_uso_resaltante(), FICHA_ds);
