...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Tipo_Via : SubFicha
    {
        internal Tipo_Via()
        {
            InitializeComponent();
        ...
        void listar()
        {
            FICHA_ds = (new cTipo_Vias()).listar();
            base.refrescar();
            SUBFICHA_bs.DataSource = FICHA_ds;
            SUBFICHA_bs.DataMember = "VS_LISTAR_TIPO_VIA";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                cTipo_Vias tipo_vias = new cTipo_Vias(); 
                tipo_vias.Codigo = codigo_txt.Text;
                tipo_vias.Descripcion = descripcion_txt.Text;
                tipo_vias.Usuario = Usuario;
                i = tipo_vias.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, el Tipo de Vía : '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                tipo_vias.Descripcion = descripcion_txt.Text;
                tipo_vias.Usuario = Usuario;
                i = tipo_vias.modificar();
                if (i > 0) MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente, el tipo de Vía esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void eliminar()
        {
            ...
            tipo_vias.Codigo = codigo_txt.Text;
            tipo_vias.Usuario = Usuario;
            i = tipo_vias.eliminar();
            if (i >= 2) MessageBox.Show("Se a eliminado el Registro.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else MessageBox.Show("Error, No se puede eliminar, el tipo de Vía esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void listar_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                codigo_txt.DataBindings.Add(new Binding("Text", SUBFICHA_bs, "TIPO DE VIA", true));
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_TIPO_VIA.DefaultView;
                vista.RowFilter = query;
                DataTable tabla = vista.ToTable();
                Reporte reporte;
                if (tabla.Rows.Count > 0) reporte = new Reporte(new tipo_via(), tabla);
                else reporte = new Reporte(new tipo_via(), FICHA_ds);
