...
namespace Formularios
{
    ...
    /// </summary>
    internal partial class Uso_Codigo_General : SubFicha
    {
       internal Uso_Codigo_General()
        {
            InitializeComponent();
        ...
        void listar()
        {
            ...
            SUBFICHA_bs.DataMember = "VS_LISTAR_USO_CODIGO_GENERAL";
        }
        void guardar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                cUso_Codigo_General uso_codigo_general = new cUso_Codigo_General();
                uso_codigo_general.Codigo = codigo_txt.Text;
                uso_codigo_general.Descripcion = descripcion_txt.Text;
                uso_codigo_general.Usuario = Usuario;
                i = uso_codigo_general.guardar();
                if (i > 0) MessageBox.Show("Se a Insertando Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, el Código de uso general : '" + codigo_txt.Text + "', ya existe en el base de datos.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();
            }
        }
        void modificar()
        {
            if (codigo_txt.Text.Length > 0 && descripcion_txt.Text.Length > 0)
            {
                ...
                uso_codigo_general.Descripcion = descripcion_txt.Text;
                uso_codigo_general.Usuario = Usuario;
                i = uso_codigo_general.modificar();

                if (i > 0) MessageBox.Show("Se a Modificado Correctamente.", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else MessageBox.Show("Error, no se a Modificado Correctamente, Uso General esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                iniciar();       
            }
        }
        void eliminar()
        {
            ...
            uso_codigo_general.Codigo = codigo_txt.Text;
            uso_codigo_general.Usuario = Usuario;
            i = uso_codigo_general.eliminar();
            if (i >= 2) MessageBox.Show("Se a eliminado el Registro", "Correcto", MessageBoxButtons.OK, MessageBoxIcon.Information);            
            else MessageBox.Show("Error, No se puede eliminar, uso de codigo general esta en uso y/o esta Desabilitada.", "Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
            iniciar();
        }
        #endregion
        ...
        private void vistaPrevia_tsb_Click(object sender, EventArgs e)
        {
            try
            {
                DataView vista = FICHA_ds.VS_LISTAR_USO_CODIGO_GENERAL.DefaultView;
